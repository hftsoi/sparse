#ifndef NNET_CONV1D_MASK_LATENCY_H_
#define NNET_CONV1D_MASK_LATENCY_H_
...
void conv_1d_mask_latency_cl(data_T data[CONFIG_T::in_width * CONFIG_T::n_chan],
                             ap_uint<1> mask[CONFIG_T::in_width],
...
PartitionLoop:
    ...
    PixelLoop:
        for (unsigned i_pxl = 0; i_pxl < CONFIG_T::n_pixels; i_pxl++) {
            ...
            // Check the mask for the pixel (sequence)
            if (mask[i_pxl] == 0) {
                // If it is mask, assign zeros directly to output channels, skipping the computations below
            ...
            Product1:
                for (int i_in = 0; i_in < mult_n_in; i_in++) {
                    #pragma HLS UNROLL
                    data_T cache = data_buf[i_pxl][i_in];
                Product2:
                    for (int i_out = 0; i_out < mult_n_out; i_out++) {
                        #pragma HLS UNROLL
